...
var _ = Describe("operator", func() {
	...
	var _ = Describe("Configuration", func() {
		...
		Context("Resource Injector", func() {
			...
			Context("Multiple sriov device and attachment", func() {
				...
				It("Should configure multiple network attachments", func() {
					...
					testNode, _ := bootstrapSriovEnv("sriovnic", sriovNetworkName, sriovInfos, false, ipam)
			...
			Context("IPv6 configured secondary interfaces on pods", func() {
				...
				It("should be able to ping each other", func() {
					...
					testNode, _ := bootstrapSriovEnv("sriovnic", ipv6NetworkName, sriovInfos, false, ipam)
		...
		Context("unhealthyVfs", func() {
			// 25834
			FIt(" Should not be able to create pod successfully if there are only unhealthy vfs", func() {
				resourceName := "sriovnic"
				sriovNetworkName := "sriovnetwork"
				ipam := `{"type": "host-local","ranges": [[{"subnet": "3ffe:ffff:0:01ff::/64"}]],"dataDir": "/run/my-orchestrator/container-ipam-state"}`
				testNode, sriovDevice := bootstrapSriovEnv(resourceName, sriovNetworkName, sriovInfos, true, ipam)
...
func bootstrapSriovEnv(resourceName string, sriovNetworkName string, sriovInfos *cluster.EnabledNodes, sriovDeviceUnused bool, ipam string) (node string, sriovDevice *sriovv1.InterfaceExt) {
